...
func get_colors():
	return get_colors_from_shader($Star.material)

func set_colors(colors):
	set_colors_on_shader($Star.material, colors)
...
func randomize_colors():
	...
	var hue_base = randf_range(0.0, 0.15) # Warm colors for stars
	var stops = []
	for i in range(num_stops):
		var hue = fmod(hue_base + float(i) / float(num_stops) * 0.1, 1.0)
		var sat = randf_range(0.6, 1.0)
		var val = randf_range(0.5 + float(i) / float(num_stops) * 0.5, 1.0)
